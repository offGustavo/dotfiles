...
function M.setup(user_opts)
    ...
    if opts.enable_cmds then
        vim.cmd('command! Lazygit lua require("forge.lazygit").open()')
        -- vim.keymap.set("n", "<leader>gg", function()
        --     require("forge.lazygit").open(vim.loop.cwd())
        -- end, { desc = "Open lazygit" })

        -- Optional: commands that open lazygit in different directories
        -- e.g., current file's directory, Neovim's cwd, etc.
        vim.cmd('command! LazygitCwd lua require("forge.lazygit").open(vim.loop.cwd())')
        vim.cmd('command! LazygitFileDir lua require("forge.lazygit").open(vim.fn.expand("%:p:h"))')
    end
end
