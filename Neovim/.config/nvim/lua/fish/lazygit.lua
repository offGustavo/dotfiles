...
-- Função para extrair cores dos highlights do Neovim
function M.get_theme_colors()
  ...
  -- Fallbacks para cores não encontradas (baseado no tema Tokyo Night)
...
-- Gera o conteúdo YAML para o tema do lazygit
...
-- Escreve o arquivo de configuração do lazygit
function M.update_lazygit_theme()
  ...
  if file then
    ...
    vim.notify("❌ Failed to write lazygit theme config", vim.log.levels.ERROR)
    return false
  end
end

-- Configura os autocommands para sincronização automática
function M.setup()
  ...
  -- vim.notify("🚀 Lazygit Theme Sync loaded! Theme will update automatically.")
end
