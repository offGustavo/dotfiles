...
local function get_tab_cwd(tabnr)
  ...
  if tab_cwd == "" then tab_cwd = vim.fn.getcwd(-1, tabnr) end
  local cwd_short = vim.fn.fnamemodify(tab_cwd, ":~:t")
  if cwd_short == "" then cwd_short = "~" end
  return cwd_short
end
...
local function get_unique_name(buf, all_names)
  ...
  if base_name == "" then base_name = "[No Name]" end
...
local function get_window_names(tabnr)
  ...
  for _, buf in ipairs(buflist) do
    ...
    local display_name
    -- Check if there are multiple buffers with this base name
    if name_groups[base_name] and #name_groups[base_name] > 1 then
      -- Multiple buffers share this name, need to make it unique
      display_name = get_unique_name(buf, name_groups)
    else
      display_name = base_name
    end
    
    -- Avoid duplicates in the final list (in case of multiple windows showing same buffer)
